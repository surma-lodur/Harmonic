...
#ifdef DEBUG
  Serial.println("Harmonic.calculate();");
#endif
  for (unsigned short note = 0; note < max_notes; note++) {
    NOTES[note] = calculateHertz(note + offset);
    PERIODS[note] = (unsigned int)((1 / NOTES[note]) * factor);
  }
}
static void calculate(unsigned short offset) { calculate(offset, pow(10, 6)); }

[[maybe_unused]] static void calculate() { calculate(0); }
} // namespace Harmonic
